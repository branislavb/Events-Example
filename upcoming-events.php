<?php
/**
 * Plugin Name: Events BB
 * Plugin URI: http://branislavborojevic.com
 * Description: A plugin to show a list of Events on the Front-End.
 * Version: 1.0
 * Author: Branislav Borojevic
 * Author URI: http://branislavborojevic.com
 * License: GPL2
 */
...
function uep_render_event_info_metabox( $post ) {
	...
	<p> 
		...
		<input type="text" id="uep-event-start-date" name="uep-event-start-date" class="widefat uep-event-date-input" value="<?php echo date( 'F d, Y', $event_start_date ); ?>" placeholder="Format: June 01, 2017">
	</p>
	<p>
		<label for="uep-event-end-date"><?php _e( 'Event End Date:', 'uep' ); ?></label>
		<input type="text" id="uep-event-end-date" name="uep-event-end-date" class="widefat uep-event-date-input" value="<?php echo date( 'F d, Y', $event_end_date ); ?>" placeholder="Format:  June 01, 2017">
	</p>
	<p>
...
function my_custom_template($single) {
   ...
   if ( 'event' == $_POST['post_type'] ) {
        if(file_exists(PLUGIN_PATH . '/templates/single-event.php'))
            return PLUGIN_PATH . '/templates/single-event.php';
    }
    return $single;
}
